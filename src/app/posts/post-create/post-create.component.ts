...
export class PostCreateComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.form = new FormGroup({
      'title': new FormControl(null, {validators: [Validators.required, Validators.minLength(2)]}, null),
      'image': new FormControl(null, {asyncValidators: [mimeType]}, null),
      'content': new FormControl(null, {validators: [Validators.required]}, null)
    });
    ...
    this.authStatusListenerSubs = this.authService.getAuthStatusListener().subscribe(authStatus => {
  ...
  onImageSelected(event: Event) {
    ...
    if (this.fileTypeChecker(file).match(/jpg/i) ||
  ...
  // getControls() {
  //   return (<FormArray>this.form.get('replies')).controls;
  // }
}
